function decode(blueprintString) {
  return JSON.parse(
    String.fromCharCode.apply(
      null,
      pako.inflate(base64js.toByteArray(blueprintString.slice(1))),
    ),
  )
}
...
function encode(blueprintObject) {
  return (
    '0' + base64js.fromByteArray(pako.deflate(JSON.stringify(blueprintObject)))
  )
}
...
function getSignals(data) {
  ...
  signals[constantNum] = [
    ...
    {
      signal: { type: 'virtual', name: 'signal-4' },
      count: data.delays[4],
      index: 5,
    },
  ]
  ...
  for (i in data.signals) {
    ...
    if (signals[constantNum] === undefined) signals[constantNum] = []

    signals[constantNum].push({
      signal: factorio_signals[i],
      count: data.signals[i],
      index: index,
    })
...
function addMemoryCell(entities, position, data) {
  ...
  if (data.num == 0) {
    connections = {
      1: {
        red: [{ entity_id: 19, circuit_id: 2 }],
      },
      2: {
        green: [{ entity_id: 17, circuit_id: 1 }],
      },
    }
    ...
    connections = {
      1: {
        red: [{ entity_id: columnHeadEntityId, circuit_id: 1 }],
      },
      2: {
        green: [{ entity_id: columnHeadEntityId, circuit_id: 2 }],
      },
    }
    ...
    connections = {
      1: {
        red: [{ entity_id: lastPlacedDeciderEntityId, circuit_id: 1 }],
      },
      2: {
        green: [{ entity_id: lastPlacedDeciderEntityId, circuit_id: 2 }],
      },
    }
  ...
  for (var constantNum in signals) {
    entities.push({
      ...
      connections: {
        1: {
          green: [{ entity_id: entities.length, circuit_id: 1 }],
        },
      },
    })
    position.y++
...
function getBlueprint() {
  ...
  for (var i = 0; i < signals.delays.length; i += 5) {
    ...
    for (var j = 0; j < 5; j++) {
      ...
      if (data.delays[j] === undefined) data.delays[j] = 0

      for (var k in signals.factorioSignals) {
        if (signals.factorioSignals[k][data.delays[j]] !== undefined) {
          if (data.signals[k] === undefined) data.signals[k] = 0

          data.signals[k] +=
            signals.factorioSignals[k][data.delays[j]] << (j * 6)
        }
      }
    }

    position.y += addMemoryCell(
      entities,
    ...
    if (position.y > 3 + 18 * substationHeight - 1) {
      ...
      Math.round(position.x + 2.5) % 18 == 9 &&
      (position.y - 3) % 18 == 8
    ) {
      position.y += 2
    } else if (
  ...
  entities[9].control_behavior.filters[4].count =
    signals.delays[signals.delays.length - 1] + 1
