...
export const InstrumentStage = ({
  ...
  const {
    midi,
    additionalInfo: { noteExtremes, trackExtremes, totalNotes },
    settings,
  } = song
  ...
  return (
    <div className={`panel pt0 flex-column w-full ${className}`} ref={panel}>
      ...
      <div className="flex flex-col sm:flex-row gap-x-3 w-full">
        ...
        <div className="panel-inset-lighter flex-column gap-2 flex-start flex-1 min-w-32 max-w-fit basis-1/5 ">
          ...
          {midi.tracks.map((track, trackNumber) => (
            <button
              key={trackNumber}
              className={`mr0 max-w-full !min-w-0 text-ellipsis overflow-hidden ${selectedTrack === trackNumber ? 'button-green' : 'button'}`}
        ...
        {/* Track settings */}
        <div className="flex-column flex-1 basis-4/5 panel-inset">
          {selectedTrack !== undefined
            ? (() => {
                  ...
                  <>
                    ...
                    {!track.instrument.percussion && (
                      <>
                        ...
                        <p>
                          Each note will be assigned to the first instrument
                          that can play it.
                        </p>
                              ...
                              >
                                {(
                                  ...
                                  .filter((instrumentName) => {
                                    if (
                                      instrumentName === trackInstrument?.name
                                    )
                                      return true
                    ...
                    <div className="flex flex-col lg:flex-row gap-3">
                      <div className="text-justify my-3 flex-column">
                        <p>
                          The histogram shows the distribution of note
                          velocities. You can select how many velocity groups
                          you would like to round velocity values to.
                        </p>
                        <p>
                          Each Programmable Speaker is only able to play notes
                          from a single velocity group, so the more groups you
                          add the more Speakers will be included in the final
                          blueprint.
                        </p>
            ...
            : 'Global settings will be here soon'}
        </div>
      </div>
    </div>
  )
