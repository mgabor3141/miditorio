...
its facing/output edge (omitted on speakers, which have no facing). Wires carry
**connector dots** at each plug-in point and a dark **casing** under the colour
core (see _Wires_ below). It also runs two structural checks we used to do by
eye:
...
no on‑image legend; the abbreviation is printed on every box.

### Wires

Wires are drawn beneath the entity boxes and route like the game: **red**
horizontal‑then‑vertical, **green** vertical‑then‑horizontal. Three aids keep
them legible on the dark grid:

- **Casing** — each wire is a bright colour core over a wider dark stroke, so a
  wire reads cleanly over boxes and two same‑colour wires sharing a path stay
  visually separate instead of merging into one blob.
- **Connector dots** — a small bead at every endpoint, sitting exactly on the
  port the wire plugs into (its colour shows the net). This is how you read
  _which_ connector a wire uses and spot a shared node.
- **Self‑loop bump** — a wire whose two connectors are on the same edge/net
  (e.g. the volume memory's green in→out bridge) would collapse to a
  zero‑length line; it is drawn as a small square bump out the side so the
